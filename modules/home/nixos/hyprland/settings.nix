...
in {
  wayland.windowManager.hyprland.settings = {
    # The Lua backend needs a local variable for composed bind strings.
    mod._var = "SUPER";

    # Home Manager renders this Nix attrset as hl.config({ ... }).
    config = {
      ...
      general = {
        ...
        col = {
          active_border = {
            colors = [
              "rgb(${config.lib.stylix.colors.base08})"
              "rgb(${config.lib.stylix.colors.base0C})"
            ];
            angle = 45;
          };
